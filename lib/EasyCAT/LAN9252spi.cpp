...
//#if defined(ARDUINO_ARCH_AVR)
  ...
  SPI_H.begin(14, 12, 13, SCS);
  digitalWrite(SCS, HIGH);
  pinMode(SCS, OUTPUT);   
  
  delay(100);     
  SPI_H.beginTransaction(SPISettings(SpiSpeed, MSBFIRST, SPI_MODE0)); 
}
...
void SPI_TransferTx(unsigned char Data) { SPI_H.transfer(Data); };
void SPI_TransferTxLast(unsigned char Data) { SPI_H.transfer(Data); };
unsigned char SPI_TransferRx(unsigned char Data) { return SPI_H.transfer(Data); };